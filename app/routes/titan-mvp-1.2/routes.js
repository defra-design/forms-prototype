  ...
  if (checkBeforeSubmission === "yes") {
    parts.push(
      whoCanCheckDescription
        ? `Checked by ${whoCanCheckDescription}`
        : "Check before submission",
    );
  }

  if (reusePreviousAnswers === "yes") {
    parts.push("Saved answers on");
  }
...
router.get("/titan-mvp-1.2/form-editor/listing", function (req, res) {
  ...
  res.render("titan-mvp-1.2/form-editor/listing/index", {
    formPages,
    sections,
    advancedSettingsHint: getListingAdvancedSettingsHint(formData),
    form: {
      name: formData.formName || "Form name",
    },
    request: req,
...
function renderAdvancedSettingsOverview(req, res) {
  ...
  res.render("titan-mvp-1.2/form-editor/advanced-settings", {
    summaryRows: getAdvancedSettingsSummaryRows(formData),
    form: {
      name: formData.formName || "Form name",
    },
    saved: req.query.saved === "true",
...
router.get(
  "/titan-mvp-1.2/form-editor/advanced-settings",
...
function renderAdvancedSettingsChange(req, res) {
  const formData = req.session.data || {};
  const setting = ADVANCED_SETTINGS_PAGES[req.params.setting];

  if (!setting) {
    return res.redirect("/titan-mvp-1.2/form-editor/advanced-settings");
  }

  res.render("titan-mvp-1.2/form-editor/advanced-settings/change", {
    data: getAdvancedSettings(formData),
    setting,
    form: {
      name: formData.formName || "Form name",
    },
...
router.get(
  "/titan-mvp-1.2/form-editor/advanced-settings/:setting.html",
  renderAdvancedSettingsChange
);
...
router.post(
  ...
  function (req, res) {
    ...
    const selectedValue = req.body[setting.key] || "no";
    formData.advancedSettings[setting.key] = selectedValue;

    if (setting.slug === "check-before-submission") {
    ...
    res.redirect(
      "/titan-mvp-1.2/form-editor/advanced-settings?saved=true"
    );
  }
);
...
function getRunnerSignInResumeStepId(application) {
  ...
  const answers = application && typeof application.answers === "object" && application.answers ? application.answers : {};
  ...
  // Prefer resuming at the first "logical" unanswered step.
  for (const step of formDef.steps || []) {
    const fields = Array.isArray(step.fields) ? step.fields : [];
    const allAnswered = fields.length === 0 ? true : fields.every((f) => isAnswered(answers[f]));
    if (!allAnswered) return step.id;
  }

  // If everything is answered, fall back to the last step (or the saved step if valid).
  const stepId = String((application && application.step) || "").trim();
  if (stepId && getRunnerSignInStepDef(formDef, stepId)) return stepId;
  const last = (formDef.steps && formDef.steps[formDef.steps.length - 1] && formDef.steps[formDef.steps.length - 1].id) || first;
  return last;
}
...
function seedRunnerSignInApplicationsPrototype() {
  return [
    ...
    {
      id: "app-1721152526402",
      formKey: "change-contact-details",
      formName: "Change your contact details",
      reference: "J1Q-9MP-3KD",
    ...
    },
    {
      ...
      checking: { required: true, status: "awaiting_check" },
...
router.get("/runner-sign-in/applications", function (req, res) {
  ...
  if (String(req.query.classic || "").trim() === "1") {
    delete data.runnerSignInV2RedirectApplicationsToManage;
  }
  if (
    data.runnerSignInV2RedirectApplicationsToManage &&
    data.runnerSignInV2FocusFormKey &&
    data.runnerSignInV2FocusApplicationId &&
    String(req.query.classic || "").trim() !== "1"
  ) {
    const q = new URLSearchParams(req.query);
    q.delete("classic");
    const tail = q.toString() ? `?${q.toString()}` : "";
    return res.redirect(
      `/runner-sign-in-v2/forms/${encodeURIComponent(data.runnerSignInV2FocusFormKey)}/${encodeURIComponent(data.runnerSignInV2FocusApplicationId)}/manage${tail}`
    );
  }
...
router.get("/runner-sign-in/applications/:id", function (req, res) {
  ...
  // In the v2 context, submitted forms should land on the v2 manage hub (primary action is copy).
...
router.post("/runner-sign-in/forms/:formKey/:id/submit", function (req, res) {
  ...
  if (data.runnerSignInV2RedirectApplicationsToManage) {
    ...
    data.runnerSignInV2FocusFormKey = application.formKey;
  }

  // If the user is operating in the v2 "manage your form" context, show a v2-style submission confirmation.
  if (data.runnerSignInV2RedirectApplicationsToManage && data.runnerSignInV2FocusFormKey && data.runnerSignInV2FocusApplicationId) {
    setRunnerSignInV2ManageFocus(req, data.runnerSignInV2FocusFormKey, data.runnerSignInV2FocusApplicationId);
    return res.redirect(
      `/runner-sign-in-v2/forms/${encodeURIComponent(data.runnerSignInV2FocusFormKey)}/${encodeURIComponent(data.runnerSignInV2FocusApplicationId)}/submitted`
    );
  }
...
router.get("/runner-sign-in-v2/forms/:formKey/:applicationId/submitted", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  if (!data.runnerSignInAuthed) {
    return res.redirect(`/runner-sign-in-v2/sign-in/email?formKey=${encodeURIComponent(req.params.formKey)}&applicationId=${encodeURIComponent(req.params.applicationId)}`);
  }
  const application =
...
router.get("/runner-sign-in/forms/:formKey/:id/send-for-checking", function (req, res) {
  ...
  const shareCheckPath = runnerSignInV2CheckerEmailEntryPath(application.checking.reviewToken);
...
router.post("/runner-sign-in/forms/:formKey/:id/send-for-checking/contact-details", function (req, res) {
  ...
  reviewStore.set(application.checking.reviewToken, {
    ...
    checkerInviteEmail: String(application.checking.inviteEmail || "").trim().toLowerCase(),
    checkerV2: true,
    checkerV2EmailConfirmed: false,
...
router.get("/runner-sign-in/check/:token", function (req, res) {
  ...
  if (!token) return res.redirect("/runner-sign-in/check");
  return res.redirect(runnerSignInV2CheckerEmailEntryPath(token));
});
...
router.get("/runner-sign-in/checker-invite-notification", function (req, res) {
  ...
  const reviewStore = ensureReviewStore(req);
  let entry = token ? reviewStore.get(token) : null;
  const prototype = runnerSignInV2PrototypeMode(req);

  // Email preview URLs are often opened without server-side store state (restart, copied link).
  // prototype=1 can render the email without a live reviewStore entry (link targets why-sign-in).
  if (prototype && token && !entry) {
    entry = {
      formName: "Apply for a small grant",
      expires: Date.now() + 30 * 24 * 60 * 60 * 1000,
      checkerInviteEmail: "checker@example.com",
    };
  }

  const tokenValid = Boolean(
    entry &&
      (prototype ? runnerSignInV2ReviewEntryValid(req, entry) : Boolean(entry.expires && entry.expires > Date.now()))
  );

  const baseUrl =
    prototype && req.get && req.get("host") ? `${req.protocol}://${req.get("host")}` : PUBLIC_BASE_URL;
  const checkUrl = tokenValid ? `${baseUrl}${runnerSignInV2CheckerEmailEntryPath(token)}` : "";

  const toEmail =
    tokenValid && entry && entry.checkerInviteEmail ? String(entry.checkerInviteEmail).trim() : "";
...
router.post("/runner-sign-in/applications/:id/clone", function (req, res) {
  ...
  // After copying, take the user to check answers (runner-v5 pattern).
  // This is useful in the v2 manage hub flow where the primary action is "Copy".
  if (data.runnerSignInV2RedirectApplicationsToManage) {
    setRunnerSignInV2ManageFocus(req, cloned.formKey, cloned.id);
  }
  return res.redirect(
    `/runner-sign-in/forms/${encodeURIComponent(cloned.formKey)}/${encodeURIComponent(cloned.id)}/check-answers?copied=1`
  );
});
...
function runnerSignInV2SecurityPath(formKey, applicationId) {
  return `/runner-sign-in-v2/forms/${encodeURIComponent(formKey)}/${encodeURIComponent(applicationId)}/security`;
}

function runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, step) {
  return `/runner-sign-in-v2/forms/${encodeURIComponent(formKey)}/${encodeURIComponent(applicationId)}/security/change-email/${step}`;
}

function runnerSignInV2SecurityChangePhonePath(formKey, applicationId, step) {
...
function runnerSignInV2ChangeEmailNewEmailSameAsCurrentContext(req) {
  ...
  return {
    ...
    error: { runnerSignInV2NewEmail: RUNNER_SIGN_IN_V2_CHANGE_EMAIL_SAME_AS_CURRENT_ERROR },
    values: { runnerSignInV2NewEmail: email },
  };
}
...
function runnerSignInV2EnsureSecuritySession(req, res) {
  ...
  if (!data.runnerSignInAuthed) {
    if (runnerSignInV2PrototypeMode(req)) {
      ...
      res.redirect(
        `/runner-sign-in-v2/sign-in/email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(runnerSignInV2SecurityPath(formKey, applicationId))}`
      );
      return null;
    }
  }
  const application =
    runnerSignInV2FindApplication(req, formKey, applicationId) ||
...
function setRunnerSignInV2ManageFocus(req, formKey, applicationId) {
  ...
  if (data.runnerSignInV2FocusFormKey && data.runnerSignInV2FocusApplicationId) {
    data.runnerSignInV2ManageUrl = runnerSignInV2ManagePath(
      data.runnerSignInV2FocusFormKey,
      data.runnerSignInV2FocusApplicationId
    );
    data.runnerSignInV2SecurityUrl = runnerSignInV2SecurityPath(
      data.runnerSignInV2FocusFormKey,
      data.runnerSignInV2FocusApplicationId
    );
  } else {
...
function runnerSignInV2MaskedMobile(data) {
  const digits = String((data && data.runnerSignInPhone) || "").replace(/\D+/g, "");
  return digits.length >= 4 ? digits.slice(-4) : "";
}

function runnerSignInV2NormalizePhone(value) {
  return String(value || "").replace(/\D+/g, "");
}

function runnerSignInV2FormatMobileDisplay(phone) {
  const digits = runnerSignInV2NormalizePhone(phone);
  ...
  return String(phone || "").trim() || "07700 900 000";
}
...
function runnerSignInV2RecoverNoSignInForMobilePath(formKey, applicationId, next) {
  return `/runner-sign-in-v2/recover/no-sign-in-for-mobile?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next)}`;
}

const RUNNER_SIGN_IN_V2_STATIC_RECOVER_NO_SIGN_IN_FOR_MOBILE_PATH =
  "/runner-sign-in-v2/static/recover-no-sign-in-for-mobile";
...
function runnerSignInV2RecoverNoSignInForMobileContext(req, { formKey, applicationId, next, mobile }) {
  ...
  const triple = (path) =>
    `${path}?formKey=${enc(formKey)}&applicationId=${enc(applicationId)}&next=${enc(resolvedNext)}`;
  return {
    data,
    application: runnerSignInV2ResolveApplication(req, formKey, applicationId),
    formKey,
    applicationId,
    next: resolvedNext,
    mobile: runnerSignInV2FormatMobileDisplay(mobile || "07123 456789"),
    recoverPhoneUrl: runnerSignInV2RecoverPhonePath(formKey, applicationId, resolvedNext),
    createSignInUrl: triple("/runner-sign-in-v2/create-sign-in/email"),
...
function isRunnerSignInV2ProbablyEmail(value) {
  ...
  if (!s) return false;
  // Prototype-only check: prevents phone numbers being accepted as emails.
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(s);
}

const RUNNER_SIGN_IN_V2_DEMO_SECURITY_CODE = "123456";

function validateRunnerSignInV2EmailField(email) {
  const s = String(email || "").trim();
  if (!s) return "Enter an email address";
  if (!isRunnerSignInV2ProbablyEmail(s)) {
...
function validateRunnerSignInV2MobileField(phone) {
  ...
  const international = /^\+[1-9]\d{9,14}$/.test(normalized);
  if (ukMobile || international || (digits.length >= 10 && digits.length <= 15)) {
    return null;
  }
  return "Enter telephone number in the correct format";
}

function validateRunnerSignInV2OtpField(code, { checkDemoCode } = {}) {
  const digits = String(code || "").replace(/\s+/g, "");
  if (!digits) return "Enter the 6 digit security code";
  if (digits.length > 6) return "Enter the security code using only 6 digits";
  if (!/^\d+$/.test(digits)) return "Enter the security code using only 6 digits";
  if (digits.length < 6) return "Enter the 6 digit security code";
  if (checkDemoCode && digits !== RUNNER_SIGN_IN_V2_DEMO_SECURITY_CODE) {
    return "The code you entered is not correct or has expired – enter it again or request a new code";
  }
  return null;
}

function runnerSignInV2CheckEmailRedirectPath(formKey, applicationId, next, extraQuery) {
  let url =
    `/runner-sign-in-v2/create-sign-in/check-email?formKey=${encodeURIComponent(formKey)}` +
...
function runnerSignInV2ReadTriple(req, body) {
  const src = body || req.query || {};
  return {
    formKey: String(src.formKey || "").trim(),
    applicationId: String(src.applicationId || "").trim(),
    next: sanitizeRunnerSignInNext(src.next),
  };
}

function runnerSignInV2NextIsCheckerJourney(req, next) {
  const s = sanitizeRunnerSignInNext(next);
  if (s && String(s).startsWith("/runner-sign-in-v2/checker/")) return true;
  const data = ensureRunnerSignInSession(req);
  const p = sanitizeRunnerSignInNext(data.runnerSignInV2CheckerPendingNext);
  return Boolean(p && String(p).startsWith("/runner-sign-in-v2/checker/"));
}

function runnerSignInV2PostAuthDestination(req, formKey, applicationId, next) {
  const data = ensureRunnerSignInSession(req);
  let resolved = sanitizeRunnerSignInNext(next);
  if (!resolved && data.runnerSignInV2CheckerPendingNext) {
    resolved = sanitizeRunnerSignInNext(data.runnerSignInV2CheckerPendingNext);
  }
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "/runner-sign-in-v2/start-page";
  if (resolved && runnerSignInV2IsSaveAndExitNext(resolved)) {
    return resolved;
  }
  if (resolved && String(resolved).startsWith("/runner-sign-in-v2/checker/")) {
...
function runnerSignInV2FindApplication(req, formKey, applicationId) {
  const applications = ensureRunnerSignInApplications(req);
  return applications.find((a) => a.id === applicationId && a.formKey === formKey) || null;
}

function runnerSignInV2ResolveApplication(req, formKey, applicationId) {
  const fk = String(formKey || "").trim();
  const id = String(applicationId || "").trim();
  if (!fk || !id) return null;
  return runnerSignInV2FindApplication(req, fk, id) || runnerSignInV2EnsureApplication(req, fk, id);
}

function runnerSignInV2EnsureApplication(req, formKey, applicationId) {
  const data = ensureRunnerSignInSession(req);
  const applications = ensureRunnerSignInApplications(req);
  const wantedId = String(applicationId || "").trim();
  const existing = applications.find((a) => a.id === wantedId);
  if (existing) return existing;

  const fk = String(formKey || "").trim();
  const def = getRunnerSignInFormDef(fk);
  if (!def) return null;

  const firstStepId = getRunnerSignInFirstStepId(def);
  const created = {
    id: wantedId || `app-${Date.now()}`,
    formKey: def.formKey,
  ...
  ensureRunnerSignInChecking(created);
  applications.unshift(created);
  data.runnerSignInApplications = applications;
  return created;
}

function runnerSignInV2CheckerCheckAnswersDisplay(entry) {
  const s = entry && entry.dataSnapshot && typeof entry.dataSnapshot === "object" ? entry.dataSnapshot : {};
  const formName = (entry && entry.formName) || "Apply for a small grant";
  const rawAmount = s.amountRequested;
  let amountRequested;
  if (rawAmount === undefined || rawAmount === null || rawAmount === "") {
    amountRequested = "£6,500";
  } else if (String(rawAmount).includes("£")) {
    amountRequested = String(rawAmount);
  } else {
    amountRequested = `£${rawAmount}`;
  }
  return {
    formName,
    applicantEmail: s.applicantEmail || "applicant@example.com",
    contactName: s.contactName || s.name || "Alex Morgan",
    contactEmail: s.contactEmail || s.email || "applicant@example.com",
    phoneNumber: s.phoneNumber || s.phone || "07700 900123",
    organisationName: s.organisationName || "Riverside Community Garden",
    organisationType: s.organisationType || "Charity",
    amountRequested,
    projectSummary:
      s.projectSummary ||
      "Rainwater harvesting equipment and community workshops for local primary schools.",
  };
}
...
function runnerSignInV2CheckedOnDisplay(iso) {
  ...
  const checkedDateText = d.toLocaleDateString("en-GB", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
  const checkedTimeText = d
    .toLocaleTimeString("en-GB", {
      hour: "numeric",
      minute: "2-digit",
      hour12: true,
    ...
    .replace(" ", "")
    .toLowerCase();
  return `${checkedDateText} at ${checkedTimeText}`;
}

function runnerSignInV2CheckerDeclarationConfirmed(body) {
  const d = body && body.declaration;
  if (Array.isArray(d)) return d.includes("confirmed");
  return d === "confirmed";
}
...
router.get("/runner-sign-in-v2/sign-out", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const formKey =
    String(req.query.formKey || data.runnerSignInV2FocusFormKey || "").trim();
  const applicationId =
    String(req.query.applicationId || data.runnerSignInV2FocusApplicationId || "").trim();
  ...
  let signInUrl = "/runner-sign-in-v2/sign-in/email";
  if (formKey && applicationId) {
    const manageUrl = runnerSignInV2ManagePath(formKey, applicationId);
    signInUrl = `/runner-sign-in-v2/sign-in/email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(manageUrl)}`;
  }
...
router.get("/runner-sign-in-v2/start-page", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  ...
  const applicationId = String(req.query.applicationId || "").trim();
  const manageUrl =
    formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "";
  // Ensure the seeded list includes at least one "Awaiting check" and one "Submitted"
  // example (if present), so the start page demonstrates key states.
  ...
  // Build in priority order, then fill remaining slots.
  const pickedSeedApps = [awaitingCheckExample, submittedExample, ...filler]
    .filter(Boolean)
    .filter((a, idx, arr) => arr.findIndex((b) => b && b.id === a.id) === idx)
    .slice(0, 5);

  const seedApplications = pickedSeedApps.map((a) => ({
    id: a.id,
    formKey: a.formKey,
    formName: a.formName,
    ...
    hint:
      a.status === "Submitted"
        ? "Submitted"
        : a.status === "Not yet started"
          ? "Not yet started"
          : a.checking && a.checking.required && String(a.checking.status) === "awaiting_check"
            ? "Awaiting check"
            : "In progress",
    highlight:
...
const RUNNER_SIGN_IN_V2_UNEXPECTED_PAGES = {
  ...
  "no-sign-in-found": {
    ...
    template: "titan-mvp-1.2/runner-sign-in-v2/unexpected/no-sign-in-found",
    runnerView: true,
    paragraphs: [
  ...
  "security-code-expired": {
    ...
    buttonHref: "/runner-sign-in-v2/create-sign-in/get-security-code",
  },
  ...
  "cannot-recover-online": {
    ...
    paragraphs: [
      ...
      "Contact us if you need access to a form you have already saved.",
    ],
  ...
  "service-unavailable": {
    title: "Sorry, the form is unavailable",
    journey: "Technical errors",
    paragraphs: [
      "You will be able to use the form later.",
  ...
  "problem-with-service": {
    title: "Sorry, there is a problem with the form",
    journey: "Technical errors",
    paragraphs: [
      "Try again later.",
      "We have not saved any information you entered on the previous page.",
...
function runnerSignInV2JourneyPreviewBase(req) {
  const data = ensureRunnerSignInSession(req);
  data.runnerSignInV2PrototypeMode = true;
  const formKey = RUNNER_SIGN_IN_V2_SAVE_EXIT_DEMO_FORM_KEY;
  const applicationId = RUNNER_SIGN_IN_V2_SAVE_EXIT_DEMO_APP_ID;
  const manageUrl = runnerSignInV2ManagePath(formKey, applicationId);
  const securityUrl = runnerSignInV2SecurityPath(formKey, applicationId);
  const application = runnerSignInV2EnsureApplication(req, formKey, applicationId);
  const email = "you@example.com";
  const phone = "07700 900000";
  return {
    ...
    phone,
    journeyPreview: true,
    journeysUrl: "/runner-sign-in-v2/journeys",
    error: {},
    values: {},
  };
}
...
router.get("/runner-sign-in-v2/journeys/preview/:slug", function (req, res) {
  ...
  if (slug === "create-sign-in-email") {
    return res.render("titan-mvp-1.2/runner-sign-in-v2/create-sign-in/email", {
      ...base,
      next: manageUrl,
      backUrl: journeysUrl,
      values: { runnerSignInV2Email: email },
    });
  }

  if (slug === "create-sign-in-check-email") {
    data.runnerSignInV2PendingEmail = email;
    return res.render("titan-mvp-1.2/runner-sign-in-v2/create-sign-in/check-email", {
      ...base,
      next: manageUrl,
      email,
      resend: false,
      backUrl: triple("/runner-sign-in-v2/create-sign-in/email", manageUrl),
    });
  }

  if (slug === "create-sign-in-get-security-code") {
    data.runnerSignInV2PendingEmail = email;
    return res.render("titan-mvp-1.2/runner-sign-in-v2/create-sign-in/get-security-code", {
      ...base,
      next: manageUrl,
      backUrl: triple("/runner-sign-in-v2/create-sign-in/check-email", manageUrl),
      formAction: triple("/runner-sign-in-v2/create-sign-in/get-security-code", manageUrl),
    });
  }

  if (slug === "create-sign-in-mobile") {
    data.runnerSignInV2PendingEmail = email;
    data.runnerSignInV2CreateSignInEmailConfirmed = true;
    return res.render("titan-mvp-1.2/runner-sign-in-v2/create-sign-in/mobile", {
      ...base,
      next: manageUrl,
      backUrl: triple("/runner-sign-in-v2/create-sign-in/check-email", manageUrl),
      values: { runnerSignInV2Mobile: phone },
    });
  }

  if (slug === "sign-in-email") {
  ...
  if (slug === "sign-in-check-email") {
    data.runnerSignInV2SignInPendingEmail = email;
    return res.render("titan-mvp-1.2/runner-sign-in-v2/sign-in/check-email", {
      ...base,
      next: manageUrl,
      email,
      resend: false,
      backUrl: triple("/runner-sign-in-v2/sign-in/email", manageUrl),
    });
  }
  ...
  if (slug === "why-sign-in") {
    return res.render("titan-mvp-1.2/runner-sign-in-v2/why-sign-in", {
      ...base,
      next: manageUrl,
      backUrl: `/runner-sign-in-v2/forms/${enc(formKey)}/${enc(applicationId)}/start-page`,
      optionalSignIn: true,
      continueWithoutSignInUrl: "#",
    });
  }

  if (slug === "recover-phone") {
  ...
  if (slug === "recover-get-security-code") {
    data.runnerSignInPhone = phone;
  ...
  if (slug === "recover-start") {
    ...
    return res.render("titan-mvp-1.2/runner-sign-in-v2/recover/start", {
      ...
      maskedMobile: runnerSignInV2MaskedMobile(data) || "0000",
      resend: false,
      backUrl: triple("/runner-sign-in-v2/recover/phone", manageUrl),
    });
  }

  if (slug === "recover-new-email") {
    return res.render("titan-mvp-1.2/runner-sign-in-v2/recover/new-email", {
      ...base,
      next: manageUrl,
      backUrl: triple("/runner-sign-in-v2/recover/start", manageUrl),
      values: { runnerSignInV2NewEmail: "new.email@example.com" },
    });
  }

  if (slug === "recover-check-new-email") {
    data.runnerSignInV2RecoverNewEmail = "new.email@example.com";
    return res.render("titan-mvp-1.2/runner-sign-in-v2/recover/check-new-email", {
      ...base,
      next: manageUrl,
      email: "new.email@example.com",
      resend: false,
      backUrl: triple("/runner-sign-in-v2/recover/new-email", manageUrl),
    });
  }
  ...
  if (slug === "security") {
    applyRunnerSignInV2EmailAuth(req, email, phone);
    setRunnerSignInV2ManageFocus(req, formKey, applicationId);
    return res.render("titan-mvp-1.2/runner-sign-in-v2/security", {
      data,
      application,
      formKey,
      applicationId,
  ...
  if (slug === "change-email-get-security-code") {
    applyRunnerSignInV2EmailAuth(req, email, phone);
    setRunnerSignInV2ManageFocus(req, formKey, applicationId);
    return res.render("titan-mvp-1.2/runner-sign-in-v2/security/change-email/get-security-code", {
      ...base,
      securityUrl,
      maskedMobile: runnerSignInV2MaskedMobile(data) || "0000",
      formAction: `${securityUrl}/change-email/get-security-code`,
    });
  }

  if (slug === "change-phone-get-security-code") {
    applyRunnerSignInV2EmailAuth(req, email, phone);
    setRunnerSignInV2ManageFocus(req, formKey, applicationId);
    return res.render("titan-mvp-1.2/runner-sign-in-v2/security/change-phone/get-security-code", {
      ...base,
      securityUrl,
      backUrl: triple("/runner-sign-in-v2/sign-in/check-email", manageUrl),
      formAction: `${securityUrl}/change-phone/get-security-code`,
    });
  }

  if (slug === "change-email-check-phone") {
    applyRunnerSignInV2EmailAuth(req, email, phone);
    setRunnerSignInV2ManageFocus(req, formKey, applicationId);
    return res.render("titan-mvp-1.2/runner-sign-in-v2/security/change-email/check-phone", {
      ...base,
      securityUrl,
      maskedMobile: runnerSignInV2MaskedMobile(data) || "0000",
      getSecurityCodeUrl: `${securityUrl}/change-email/get-security-code`,
      formAction: `${securityUrl}/change-email/check-phone`,
      resend: false,
    });
  }

  if (slug === "change-email-new-email") {
    applyRunnerSignInV2EmailAuth(req, email, phone);
    setRunnerSignInV2ManageFocus(req, formKey, applicationId);
    return res.render("titan-mvp-1.2/runner-sign-in-v2/security/change-email/new-email", {
      ...base,
      securityUrl,
      checkPhoneUrl: `${securityUrl}/change-email/check-phone`,
      formAction: `${securityUrl}/change-email/new-email`,
      values: { runnerSignInV2NewEmail: "new.email@example.com" },
    });
  }
  ...
  if (slug === "change-email-check-new-email") {
    applyRunnerSignInV2EmailAuth(req, email, phone);
    setRunnerSignInV2ManageFocus(req, formKey, applicationId);
    return res.render("titan-mvp-1.2/runner-sign-in-v2/security/change-email/check-new-email", {
      ...base,
      securityUrl,
      email: "new.email@example.com",
      newEmailUrl: `${securityUrl}/change-email/new-email`,
      resendUrl: `${securityUrl}/change-email/check-new-email?resend=1`,
      formAction: `${securityUrl}/change-email/check-new-email`,
      resend: false,
    });
  }

  if (slug === "change-phone-check-email") {
    applyRunnerSignInV2EmailAuth(req, email, phone);
    setRunnerSignInV2ManageFocus(req, formKey, applicationId);
    return res.render("titan-mvp-1.2/runner-sign-in-v2/security/change-phone/check-email", {
      ...base,
      securityUrl,
      email,
      getSecurityCodeUrl: `${securityUrl}/change-phone/get-security-code`,
      formAction: `${securityUrl}/change-phone/check-email`,
      resend: false,
    });
  }

  if (slug === "change-phone-new-phone") {
    applyRunnerSignInV2EmailAuth(req, email, phone);
    setRunnerSignInV2ManageFocus(req, formKey, applicationId);
    return res.render("titan-mvp-1.2/runner-sign-in-v2/security/change-phone/new-phone", {
      ...base,
      securityUrl,
      checkEmailUrl: `${securityUrl}/change-phone/check-email`,
      formAction: `${securityUrl}/change-phone/new-phone`,
      values: { runnerSignInV2Mobile: phone },
    });
  }

  if (slug === "save-exit-choose") {
    const saveExitLeaveUrl = runnerSignInV2SaveAndExitWithoutSignInLeavePath(formKey, applicationId);
    const chooseUrl = runnerSignInV2SaveAndExitChoosePath(formKey, applicationId);
    ...
    return res.render("titan-mvp-1.2/runner-sign-in-v2/save-and-exit/without-sign-in/choose", {
      ...
      backUrl: journeysUrl,
      createSignInUrl:
        `/runner-sign-in-v2/create-sign-in/email?formKey=${enc(formKey)}&applicationId=${enc(applicationId)}` +
        `&next=${enc(saveExitLeaveUrl)}&back=${enc(chooseUrl)}`,
      signInUrl:
        `/runner-sign-in-v2/sign-in/email?formKey=${enc(formKey)}&applicationId=${enc(applicationId)}` +
        `&next=${enc(saveExitConfirmUrl)}&back=${enc(chooseUrl)}`,
    });
  }

  if (slug === "create-sign-in-email-save-exit") {
    const saveExitLeaveUrl = runnerSignInV2SaveAndExitWithoutSignInLeavePath(formKey, applicationId);
    const chooseUrl = runnerSignInV2SaveAndExitChoosePath(formKey, applicationId);
    return res.render("titan-mvp-1.2/runner-sign-in-v2/create-sign-in/email", {
      ...base,
      next: saveExitLeaveUrl,
      backUrl: chooseUrl,
      values: { runnerSignInV2Email: email },
    });
  }

  if (slug === "create-sign-in-mobile-save-exit") {
    const saveExitLeaveUrl = runnerSignInV2SaveAndExitWithoutSignInLeavePath(formKey, applicationId);
    data.runnerSignInV2PendingEmail = email;
    data.runnerSignInV2CreateSignInEmailConfirmed = true;
    return res.render("titan-mvp-1.2/runner-sign-in-v2/create-sign-in/mobile", {
      ...base,
      next: saveExitLeaveUrl,
      backUrl: triple("/runner-sign-in-v2/create-sign-in/check-email", saveExitLeaveUrl),
      values: { runnerSignInV2Mobile: phone },
    });
  }

  if (slug === "create-sign-in-check-email-resend") {
    data.runnerSignInV2PendingEmail = email;
    return res.render("titan-mvp-1.2/runner-sign-in-v2/create-sign-in/check-email", {
      ...base,
      next: manageUrl,
  ...
  if (slug === "save-exit-with-sign-in-leave") {
    applyRunnerSignInV2EmailAuth(req, email, phone);
    return res.render("titan-mvp-1.2/runner-sign-in-v2/save-and-exit/with-sign-in/leave", {
      ...base,
      manageUrl,
    });
  }

  if (slug === "save-exit-without-sign-in-leave") {
    return res.render("titan-mvp-1.2/runner-sign-in-v2/save-and-exit/without-sign-in/leave", {
      ...base,
      manageUrl,
    });
  }

  if (slug === "email-save-exit-without-sign-in") {
    return res.render("titan-mvp-1.2/runner-sign-in-v2/emails/save-and-exit", {
      ...base,
      variant: "without-sign-in",
      toEmail: email,
      resumeUrl: `${PUBLIC_BASE_URL}/runner-sign-in-v2/save-and-exit/without-sign-in/resume?formKey=${enc(formKey)}&applicationId=${enc(applicationId)}`,
      formName: application.formName,
    });
  }

  if (slug === "email-save-exit-with-sign-in") {
    return res.render("titan-mvp-1.2/runner-sign-in-v2/emails/save-and-exit", {
      ...base,
      variant: "with-sign-in",
      toEmail: email,
      resumeUrl: `${PUBLIC_BASE_URL}/runner-sign-in-v2/save-and-exit/with-sign-in/resume?formKey=${enc(formKey)}&applicationId=${enc(applicationId)}`,
      formName: application.formName,
    });
  }
  ...
  if (slug === "save-exit-confirm-email") {
    applyRunnerSignInV2EmailAuth(req, email, phone);
    return res.render("titan-mvp-1.2/runner-sign-in-v2/save-and-exit/with-sign-in/confirm-email", {
      ...base,
      next: manageUrl,
      backUrl: journeysUrl,
      values: { runnerSignInV2ResumeEmail: email },
    });
  }

  if (slug === "save-exit-progress-saved") {
    ...
    return res.render("titan-mvp-1.2/runner-sign-in-v2/create-sign-in/created", {
      ...base,
      continueUrl: runnerSignInV2SaveAndExitWithoutSignInLeavePath(formKey, applicationId),
      isSaveAndExitJourney: true,
  ...
  if (slug === "save-exit-resume-without-sign-in") {
  ...
  if (slug === "save-exit-check-email") {
    data.runnerSignInV2PendingEmail = email;
    return res.render("titan-mvp-1.2/runner-sign-in-v2/save-and-exit/without-sign-in/check-email", {
      ...base,
      next: runnerSignInV2SaveAndExitWithoutSignInLeavePath(formKey, applicationId),
      email,
      resend: false,
      backUrl: triple("/runner-sign-in-v2/create-sign-in/email", runnerSignInV2SaveAndExitWithoutSignInLeavePath(formKey, applicationId)),
    });
  }

  if (slug === "delete-draft") {
    applyRunnerSignInV2EmailAuth(req, email, phone);
    setRunnerSignInV2ManageFocus(req, formKey, applicationId);
    return res.render("titan-mvp-1.2/runner-sign-in-v2/delete-application", {
      ...base,
      manageUrl,
    });
  }
  ...
  if (slug === "checker-why-sign-in") {
    return res.render("titan-mvp-1.2/runner-sign-in-v2/checker/why-sign-in", {
      ...base,
      token: RUNNER_SIGN_IN_V2_STATIC_CHECKED_REVIEW_TOKEN,
      next: runnerSignInV2CheckerEmailEntryPath(RUNNER_SIGN_IN_V2_STATIC_CHECKED_REVIEW_TOKEN),
      backUrl: journeysUrl,
    });
  }

  if (slug === "ready-to-submit") {
    applyRunnerSignInV2EmailAuth(req, email, phone);
  ...
  if (slug === "email-save-exit") {
    return res.render("titan-mvp-1.2/runner-sign-in-v2/emails/save-and-exit", {
      ...base,
      resumeUrl: triple("/runner-sign-in-v2/sign-in/email", manageUrl),
      formName: application.formName,
    });
  }

  if (slug === "email-confirmation") {
  ...
  if (slug === "email-checker-invite") {
    return res.render("titan-mvp-1.2/runner-sign-in-v2/emails/checker-invite-v2", {
      ...base,
      checkerUrl: runnerSignInV2CheckerEmailEntryPath(RUNNER_SIGN_IN_V2_STATIC_CHECKED_REVIEW_TOKEN),
      formName: application.formName,
      applicantEmail: email,
    });
  }
...
router.get("/runner-sign-in-v2/journeys.html", function (req, res) {
  return res.redirect("/runner-sign-in-v2/journeys");
});

router.get("/runner-sign-in-v2/journeys", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  if (typeof data.runnerSignInV2PrototypeMode === "undefined") {
    data.runnerSignInV2PrototypeMode = true;
  }
  ...
  const urls = runnerSignInV2DocsUrls();
...
router.get("/runner-sign-in-v2/journey-flowcharts", function (req, res) {
  ...
  return res.render("titan-mvp-1.2/runner-sign-in-v2/journey-flowcharts", {
    ...
    journeySelectItems,
    journeyGlanceItems,
    urls: runnerSignInV2DocsUrls(),
  });
});

router.get("/runner-sign-in-v2/unexpected-journeys.html", function (req, res) {
  return res.redirect("/runner-sign-in-v2/unexpected-journeys");
});
...
router.get("/runner-sign-in-v2/unexpected-journeys/:slug", function (req, res) {
  ...
  if (page.staticHref) return res.redirect(page.staticHref);

  if (page.runnerView && page.template) {
    const ctx = runnerSignInV2UnexpectedRunnerPageContext(req, slug);
...
function runnerSignInV2UnexpectedRunnerPageContext(req, slug) {
  const page = RUNNER_SIGN_IN_V2_UNEXPECTED_PAGES[String(slug || "")];
  if (!page || !page.runnerView) return null;

  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.query);
  const resolvedFormKey = formKey || RUNNER_SIGN_IN_V2_SAVE_EXIT_DEMO_FORM_KEY;
  const resolvedApplicationId = applicationId || RUNNER_SIGN_IN_V2_SAVE_EXIT_DEMO_APP_ID;
  const application =
    runnerSignInV2ResolveApplication(req, resolvedFormKey, resolvedApplicationId) ||
    runnerSignInV2EnsureApplication(req, resolvedFormKey, resolvedApplicationId);
  const manageUrl = runnerSignInV2ManagePath(resolvedFormKey, resolvedApplicationId);
  const resolvedNext = next || manageUrl;
  const enc = (value) => encodeURIComponent(value);
  const triple = (path) =>
    `${path}?formKey=${enc(resolvedFormKey)}&applicationId=${enc(resolvedApplicationId)}&next=${enc(resolvedNext)}`;
  const formStartUrl = `/runner-sign-in-v2/forms/${enc(resolvedFormKey)}/${enc(resolvedApplicationId)}/start-page`;

  const buttonHref = page.buttonHref ? triple(page.buttonHref.split("?")[0]) : "";
  const secondaryLinks = (page.secondaryLinks || []).map((link) => ({
    ...link,
    href: triple(String(link.href || "").split("?")[0]),
  }));

  return {
    data: {
      ...data,
      runnerSignInAuthed: false,
      runnerSignInV2ManageUrl: "",
    },
    application,
    formKey: resolvedFormKey,
    applicationId: resolvedApplicationId,
    next: resolvedNext,
    runnerStartUrl: formStartUrl,
    serviceName: application.formName,
    page: {
      ...page,
      buttonHref,
      secondaryLinks,
      backHref: triple("/runner-sign-in-v2/sign-in/email"),
    },
  };
}

function runnerSignInV2UnexpectedNoSignInFoundPath(formKey, applicationId, next) {
  const enc = (value) => encodeURIComponent(value);
  return `/runner-sign-in-v2/unexpected-journeys/no-sign-in-found?formKey=${enc(formKey)}&applicationId=${enc(applicationId)}&next=${enc(next)}`;
}

function runnerSignInV2FieldErrorPreviewContext(req, slug) {
  const item = RUNNER_SIGN_IN_V2_FIELD_ERRORS[String(slug || "")];
  if (!item) return null;
  ...
  const chooseUrl = runnerSignInV2SaveAndExitChoosePath(formKey, applicationId, "details");
  ...
  const enc = (value) => encodeURIComponent(value);
  const triple = (path, dest) =>
    `${path}?formKey=${enc(formKey)}&applicationId=${enc(applicationId)}&next=${enc(dest || manageUrl)}`;

  let backUrl = "/runner-sign-in-v2/error-messages";
  ...
  if (item.template.includes("without-sign-in/check-email")) {
    backUrl = triple("/runner-sign-in-v2/create-sign-in/email", leaveUrl);
  }
  ...
  return {
    ...
    resend: Boolean(item.extra && item.extra.resend),
  };
}

function runnerSignInV2DocsUrls() {
  const formKey = RUNNER_SIGN_IN_V2_SAVE_EXIT_DEMO_FORM_KEY;
  const applicationId = RUNNER_SIGN_IN_V2_SAVE_EXIT_DEMO_APP_ID;
  return {
    ...runnerSignInV2BuildJourneyUrls({
...
router.get("/runner-sign-in-v2/save-and-exit-stories", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  data.runnerSignInV2PrototypeMode = true;
  runnerSignInV2EnsureApplication(
    req,
    RUNNER_SIGN_IN_V2_SAVE_EXIT_DEMO_FORM_KEY,
    RUNNER_SIGN_IN_V2_SAVE_EXIT_DEMO_APP_ID
  );
  const demoFormKey = RUNNER_SIGN_IN_V2_SAVE_EXIT_DEMO_FORM_KEY;
  const demoApplicationId = RUNNER_SIGN_IN_V2_SAVE_EXIT_DEMO_APP_ID;
  const urls = {
    ...runnerSignInV2DocsUrls(),
...
router.get("/runner-sign-in-v2/error-messages", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const urls = runnerSignInV2DocsUrls();
  const errorGroups = groupRunnerSignInV2FieldErrors().map((group) => ({
    heading: group.heading,
    items: group.items.map((item) => ({
      ...item,
      errorText: item.error[item.field],
      previewHref: item.previewHref || `/runner-sign-in-v2/error-messages/${item.slug}/preview`,
    })),
  }));

  return res.render("titan-mvp-1.2/runner-sign-in-v2/error-messages", {
    data,
    errorGroups,
    urls,
  });
...
router.get("/runner-sign-in-v2/error-messages/:slug/preview", function (req, res) {
  const slug = String(req.params.slug || "").trim();
  const item = RUNNER_SIGN_IN_V2_FIELD_ERRORS[slug];
  if (!item) return res.redirect("/runner-sign-in-v2/error-messages");
  if (item.staticHref) return res.redirect(item.staticHref);

  const ctx = runnerSignInV2FieldErrorPreviewContext(req, slug);
  return res.render(item.template, ctx);
});

router.get("/runner-sign-in-v2/error-messages/:slug", function (req, res) {
  const slug = String(req.params.slug || "").trim();
  const item = RUNNER_SIGN_IN_V2_FIELD_ERRORS[slug];
  if (!item) return res.redirect("/runner-sign-in-v2/error-messages");

  const data = ensureRunnerSignInSession(req);
  return res.render("titan-mvp-1.2/runner-sign-in-v2/error-message-reference", {
    data,
    errorItem: {
      ...item,
    },
    previewUrl: item.staticHref || `/runner-sign-in-v2/error-messages/${encodeURIComponent(slug)}/preview`,
  });
});

function runnerSignInV2CheckerAllowApplicant(req) {
  const data = ensureRunnerSignInSession(req);
...
function runnerSignInV2CheckerAppendAllowApplicant(req, url) {
  if (!runnerSignInV2CheckerAllowApplicant(req)) return url;
  return url.includes("?") ? `${url}&allowApplicant=1` : `${url}?allowApplicant=1`;
}

function runnerSignInV2PrototypeMode(req) {
  const data = ensureRunnerSignInSession(req);
  const q = (req && req.query) || {};
  const b = (req && req.body) || {};
  return data.runnerSignInV2PrototypeMode === true || q.prototype === "1" || b.prototype === "1";
}

function runnerSignInV2ReviewEntryValid(req, entry) {
  if (!entry) return false;
  if (runnerSignInV2PrototypeMode(req)) return true;
  return Boolean(entry.expires && entry.expires > Date.now());
}

function runnerSignInV2EnsureCheckerReviewEntry(req, token, reviewStore) {
  let entry = token ? reviewStore.get(token) : null;
  if (runnerSignInV2PrototypeMode(req) && token && !entry) {
    entry = {
      applicationId: `prototype-${token}`,
      referenceNumber: normalizeRunnerV5ReferenceNumber(createRunnerV4StyleReferenceNumber()),
      memorableWord: normalizeRunnerSignInMemorableWord(createRunnerSignInMemorableWord()),
      formKey: "apply-small-grant",
      formName: "Apply for a small grant",
      dataSnapshot: {},
      reviewDeclarationComplete: false,
  ...
  if (entry && runnerSignInV2ReviewEntryValid(req, entry) && !entry.checkerV2) {
    entry.checkerV2 = true;
    entry.checkerV2EmailConfirmed = Boolean(entry.checkerV2EmailConfirmed);
    entry.checkerV2CodeOk = Boolean(entry.checkerV2CodeOk);
    reviewStore.set(token, entry);
  }
  return entry;
}

/** Checker invite links skip the intermediate landing page and open why-sign-in directly. */
function runnerSignInV2CheckerEmailEntryPath(token) {
  const t = encodeURIComponent(token);
  return `/runner-sign-in-v2/checker/why-sign-in?token=${t}&prototype=1&allowApplicant=1`;
}

router.get("/runner-sign-in-v2/prototype/on", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  data.runnerSignInV2PrototypeMode = true;
  const returnTo = sanitizeRunnerSignInNext(req.query && req.query.returnTo);
  return res.redirect(returnTo || "/runner-sign-in-v2/start-page");
});

router.get("/runner-sign-in-v2/prototype/off", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  data.runnerSignInV2PrototypeMode = false;
  // Turning prototype mode off should restore the "realistic" journeys.
  // Clear auth so Manage hub requires sign-in again.
  data.runnerSignInAuthed = false;
  delete data.runnerSignInEmail;
  delete data.runnerSignInMethod;
  delete data.runnerSignInPhone;
...
router.get("/runner-sign-in-v2/forms/:formKey/:applicationId/start-page", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const application =
    runnerSignInV2FindApplication(req, req.params.formKey, req.params.applicationId) ||
    runnerSignInV2EnsureApplication(req, req.params.formKey, req.params.applicationId);
...
router.get("/runner-sign-in-v2/why-sign-in", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.query);
...
router.get("/runner-sign-in-v2/create-sign-in/email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.query);
  const application = runnerSignInV2ResolveApplication(req, formKey, applicationId);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "";
  ...
  return res.render("titan-mvp-1.2/runner-sign-in-v2/create-sign-in/email", {
    ...
    values: { runnerSignInV2Email: data.runnerSignInV2PendingEmail || data.runnerSignInEmail || "" },
  });
});

router.post("/runner-sign-in-v2/create-sign-in/email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  ...
  const emailError = validateRunnerSignInV2EmailField(email);
  if (emailError) {
    data.runnerSignInV2Error = { runnerSignInV2Email: emailError };
    return res.redirect(
  ...
  if (runnerSignInV2NextIsCheckerJourney(req, next)) {
    const phone = String(data.runnerSignInPhone || "").trim() || "07700 900000";
    applyRunnerSignInV2EmailAuth(req, email, phone);
    delete data.runnerSignInV2PendingEmail;
    delete data.runnerSignInV2PendingMobile;
    delete data.runnerSignInV2CreateSignInEmailConfirmed;
    if (formKey && applicationId) setRunnerSignInV2ManageFocus(req, formKey, applicationId);
    return res.redirect(runnerSignInV2PostAuthDestination(req, formKey, applicationId, next));
  }
  ...
  delete data.runnerSignInV2CreateSignInEmailConfirmed;
  return res.redirect(
    `/runner-sign-in-v2/create-sign-in/check-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`
  );
});

router.get("/runner-sign-in-v2/create-sign-in/check-email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.query);
  const application = runnerSignInV2ResolveApplication(req, formKey, applicationId);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "";
  ...
  return res.render("titan-mvp-1.2/runner-sign-in-v2/create-sign-in/check-email", {
    ...
    resend,
    backUrl: `/runner-sign-in-v2/create-sign-in/email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`,
    error: data.runnerSignInV2Error || {},
    values: {},
  });
});

router.post("/runner-sign-in-v2/create-sign-in/check-email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.body);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "/runner-sign-in-v2/start-page";
  const resolvedNext = next || manageUrl;
  if (!data.runnerSignInV2PendingEmail) {
    return res.redirect(
      `/runner-sign-in-v2/create-sign-in/email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(resolvedNext)}`
    );
  }
  ...
  if (codeError) {
    ...
    return res.redirect(runnerSignInV2CheckEmailRedirectPath(formKey, applicationId, resolvedNext));
  }
  delete data.runnerSignInV2Error;
  data.runnerSignInV2CreateSignInEmailConfirmed = true;
  return res.redirect(
    `/runner-sign-in-v2/create-sign-in/mobile?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(resolvedNext)}`
  );
});

router.get("/runner-sign-in-v2/create-sign-in/get-security-code", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.query);
  const application = runnerSignInV2ResolveApplication(req, formKey, applicationId);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "";
  ...
  const checkEmailUrl = `/runner-sign-in-v2/create-sign-in/check-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(resolvedNext)}`;
  return res.render("titan-mvp-1.2/runner-sign-in-v2/create-sign-in/get-security-code", {
    data,
    application,
    formKey,
    ...
    next: resolvedNext,
    email: data.runnerSignInV2PendingEmail,
    backUrl: checkEmailUrl,
    formAction: "/runner-sign-in-v2/create-sign-in/get-security-code",
  });
});

router.post("/runner-sign-in-v2/create-sign-in/get-security-code", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.body);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "/runner-sign-in-v2/start-page";
  const resolvedNext = next || manageUrl;
  ...
  return res.redirect(
    `/runner-sign-in-v2/create-sign-in/check-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(resolvedNext)}&resend=1`
  );
});

router.get("/runner-sign-in-v2/create-sign-in/mobile", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.query);
  const application = runnerSignInV2ResolveApplication(req, formKey, applicationId);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "";
  ...
  if (!data.runnerSignInV2CreateSignInEmailConfirmed) {
    return res.redirect(
      `/runner-sign-in-v2/create-sign-in/check-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`
    );
  }
  return res.render("titan-mvp-1.2/runner-sign-in-v2/create-sign-in/mobile", {
    ...
    applicationId,
    next: next || manageUrl,
    backUrl: `/runner-sign-in-v2/create-sign-in/check-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`,
    error: data.runnerSignInV2Error || {},
    values: { runnerSignInV2Mobile: data.runnerSignInV2PendingMobile || "" },
  });
});

router.post("/runner-sign-in-v2/create-sign-in/mobile", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.body);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "/runner-sign-in-v2/start-page";
  if (!data.runnerSignInV2PendingEmail || !data.runnerSignInV2CreateSignInEmailConfirmed) {
    return res.redirect(
      `/runner-sign-in-v2/create-sign-in/email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`
    );
  }
  ...
  if (phoneError) {
    ...
    return res.redirect(
      `/runner-sign-in-v2/create-sign-in/mobile?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`
    );
  }
  delete data.runnerSignInV2Error;
  ...
  if (isSaveAndExitJourney) {
    saveRunnerSignInV2AccountWithoutSignIn(req, data.runnerSignInV2PendingEmail, phone);
    delete data.runnerSignInV2PendingEmail;
    delete data.runnerSignInV2PendingMobile;
    delete data.runnerSignInV2CreateSignInEmailConfirmed;
    if (formKey && applicationId) {
  ...
  applyRunnerSignInV2EmailAuth(req, data.runnerSignInV2PendingEmail, phone);
  delete data.runnerSignInV2PendingEmail;
  delete data.runnerSignInV2PendingMobile;
  delete data.runnerSignInV2CreateSignInEmailConfirmed;
  if (formKey && applicationId) setRunnerSignInV2ManageFocus(req, formKey, applicationId);
...
router.get("/runner-sign-in-v2/create-sign-in/created", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.query);
  const application = runnerSignInV2ResolveApplication(req, formKey, applicationId);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "/runner-sign-in-v2/start-page";
...
router.get("/runner-sign-in-v2/sign-in/email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.query);
  const application = runnerSignInV2ResolveApplication(req, formKey, applicationId);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "";
...
router.post("/runner-sign-in-v2/sign-in/email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.body);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "/runner-sign-in-v2/start-page";
  const email = String(req.body.runnerSignInV2Email || "").trim();
  ...
  if (emailError) {
    data.runnerSignInV2Error = { runnerSignInV2Email: emailError };
    return res.redirect(
      `/runner-sign-in-v2/sign-in/email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`
    );
  }
  delete data.runnerSignInV2Error;

  if (String(req.query.scenario || req.body.scenario || "").trim() === "no-sign-in") {
    return res.redirect(
      runnerSignInV2UnexpectedNoSignInFoundPath(formKey, applicationId, next || manageUrl)
    );
  }
  const knownEmail = String(data.runnerSignInEmail || "").trim().toLowerCase();
  if (
    knownEmail &&
    email.toLowerCase() !== knownEmail &&
    !data.runnerSignInAuthed &&
    String(data.runnerSignInPhone || "").trim()
  ) {
    return res.redirect(
      runnerSignInV2UnexpectedNoSignInFoundPath(formKey, applicationId, next || manageUrl)
    );
  ...
  if (runnerSignInV2NextIsCheckerJourney(req, next)) {
    applyRunnerSignInV2EmailAuth(req, email, data.runnerSignInPhone);
    delete data.runnerSignInV2SignInPendingEmail;
    if (formKey && applicationId) setRunnerSignInV2ManageFocus(req, formKey, applicationId);
    return res.redirect(runnerSignInV2PostAuthDestination(req, formKey, applicationId, next));
  }
  ...
  data.runnerSignInV2SignInPendingEmail = email;
  return res.redirect(
    `/runner-sign-in-v2/sign-in/check-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`
  );
});

router.get("/runner-sign-in-v2/sign-in/check-email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.query);
  const application = runnerSignInV2ResolveApplication(req, formKey, applicationId);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "";
  if (!data.runnerSignInV2SignInPendingEmail) {
    return res.redirect(
      `/runner-sign-in-v2/sign-in/email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`
    );
  }
  ...
  return res.render("titan-mvp-1.2/runner-sign-in-v2/sign-in/check-email", {
    ...
    applicationId,
    next: next || manageUrl,
    email: data.runnerSignInV2SignInPendingEmail,
    resend,
    backUrl: `/runner-sign-in-v2/sign-in/email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`,
    error: data.runnerSignInV2Error || {},
    values: {},
  });
});

router.post("/runner-sign-in-v2/sign-in/check-email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.body);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "/runner-sign-in-v2/start-page";
  const resolvedNext = next || manageUrl;
  ...
  if (codeError) {
    data.runnerSignInV2Error = { runnerSignInV2Code: codeError };
    return res.redirect(
      `/runner-sign-in-v2/sign-in/check-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(resolvedNext)}`
    );
  }
  delete data.runnerSignInV2Error;
  applyRunnerSignInV2EmailAuth(req, data.runnerSignInV2SignInPendingEmail, data.runnerSignInPhone);
  delete data.runnerSignInV2SignInPendingEmail;
  if (formKey && applicationId) setRunnerSignInV2ManageFocus(req, formKey, applicationId);
  return res.redirect(runnerSignInV2PostAuthDestination(req, formKey, applicationId, next));
});

router.get("/runner-sign-in-v2/sign-in/get-security-code", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.query);
  const application = runnerSignInV2ResolveApplication(req, formKey, applicationId);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "";
  ...
  if (!data.runnerSignInV2SignInPendingEmail) {
    return res.redirect(
      `/runner-sign-in-v2/sign-in/email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(resolvedNext)}`
    );
  }
  const checkEmailUrl = `/runner-sign-in-v2/sign-in/check-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(resolvedNext)}`;
  return res.render("titan-mvp-1.2/runner-sign-in-v2/sign-in/get-security-code", {
    data,
    application,
    formKey,
    ...
    email: data.runnerSignInV2SignInPendingEmail,
    backUrl: checkEmailUrl,
    formAction: "/runner-sign-in-v2/sign-in/get-security-code",
  });
});

router.post("/runner-sign-in-v2/sign-in/get-security-code", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.body);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "/runner-sign-in-v2/start-page";
  const resolvedNext = next || manageUrl;
  ...
  return res.redirect(
    `/runner-sign-in-v2/sign-in/check-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(resolvedNext)}&resend=1`
  );
});
...
router.get("/runner-sign-in-v2/recover/no-sign-in-for-mobile", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.query);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "";
  const resolvedNext = next || manageUrl;
  const mobile = data.runnerSignInV2RecoverPhoneNotFound || req.query.mobile || "07123 456789";
  return res.render(
    "titan-mvp-1.2/runner-sign-in-v2/recover/no-sign-in-for-mobile",
    runnerSignInV2RecoverNoSignInForMobileContext(req, {
      formKey,
      applicationId,
      next: resolvedNext,
      mobile,
...
router.post("/runner-sign-in-v2/recover/get-security-code", function (req, res) {
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.body);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "/runner-sign-in-v2/start-page";
  const resolvedNext = next || manageUrl;
...
router.post("/runner-sign-in-v2/recover/check-mobile", function (req, res) {
  ...
  delete data.runnerSignInV2Error;
  return res.redirect(
    `/runner-sign-in-v2/recover/new-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`
  );
});

router.get("/runner-sign-in-v2/recover/new-email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.query);
  const application = runnerSignInV2ResolveApplication(req, formKey, applicationId);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "";
  return res.render("titan-mvp-1.2/runner-sign-in-v2/recover/new-email", {
    ...
    applicationId,
    next: next || manageUrl,
    backUrl: `/runner-sign-in-v2/recover/check-mobile?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`,
    error: data.runnerSignInV2Error || {},
    values: {},
  });
});

router.post("/runner-sign-in-v2/recover/new-email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.body);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "/runner-sign-in-v2/start-page";
  const email = String(req.body.runnerSignInV2NewEmail || "").trim();
  if (!email) {
    data.runnerSignInV2Error = { runnerSignInV2NewEmail: "Enter your new email address" };
    return res.redirect(
      `/runner-sign-in-v2/recover/new-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`
    );
  }
  if (!isRunnerSignInV2ProbablyEmail(email)) {
    data.runnerSignInV2Error = { runnerSignInV2NewEmail: "Enter an email address in the correct format, like name@example.com" };
    return res.redirect(
      `/runner-sign-in-v2/recover/new-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`
    );
  }
  delete data.runnerSignInV2Error;
  data.runnerSignInV2RecoverNewEmail = email;
  return res.redirect(
    `/runner-sign-in-v2/recover/check-new-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`
  );
});

router.get("/runner-sign-in-v2/recover/check-new-email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.query);
  const application = runnerSignInV2ResolveApplication(req, formKey, applicationId);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "";
  if (!data.runnerSignInV2RecoverNewEmail) {
    return res.redirect(
      `/runner-sign-in-v2/recover/new-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`
    );
  }
  ...
  return res.render("titan-mvp-1.2/runner-sign-in-v2/recover/check-new-email", {
    ...
    applicationId,
    next: next || manageUrl,
    email: data.runnerSignInV2RecoverNewEmail,
    resend,
    backUrl: `/runner-sign-in-v2/recover/new-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(next || manageUrl)}`,
    error: data.runnerSignInV2Error || {},
    values: {},
  });
});

router.post("/runner-sign-in-v2/recover/check-new-email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId, next } = runnerSignInV2ReadTriple(req, req.body);
  const manageUrl = formKey && applicationId ? runnerSignInV2ManagePath(formKey, applicationId) : "/runner-sign-in-v2/start-page";
  const code = String(req.body.runnerSignInV2Code || "").replace(/\s+/g, "");
  ...
  delete data.runnerSignInV2Error;
  applyRunnerSignInV2EmailAuth(req, data.runnerSignInV2RecoverNewEmail, data.runnerSignInPhone);
  delete data.runnerSignInV2RecoverNewEmail;
  if (formKey && applicationId) setRunnerSignInV2ManageFocus(req, formKey, applicationId);
  return res.redirect(runnerSignInV2PostAuthDestination(req, formKey, applicationId, next));
});
...
router.get("/runner-sign-in-v2/static/recover-no-sign-in-for-mobile", function (req, res) {
  return res.render(
    "titan-mvp-1.2/runner-sign-in-v2/recover/no-sign-in-for-mobile",
    runnerSignInV2RecoverNoSignInForMobileContext(req, {
      formKey: RUNNER_SIGN_IN_V2_SAVE_EXIT_DEMO_FORM_KEY,
      applicationId: RUNNER_SIGN_IN_V2_SAVE_EXIT_DEMO_APP_ID,
      next: runnerSignInV2ManagePath(
        RUNNER_SIGN_IN_V2_SAVE_EXIT_DEMO_FORM_KEY,
        RUNNER_SIGN_IN_V2_SAVE_EXIT_DEMO_APP_ID
      ),
      mobile: "07123 456789",
...
router.get("/runner-sign-in-v2/static/change-email-new-email-same-as-current", function (req, res) {
  return res.render(
    "titan-mvp-1.2/runner-sign-in-v2/security/change-email/new-email",
    runnerSignInV2ChangeEmailNewEmailSameAsCurrentContext(req)
  );
});
...
router.get("/runner-sign-in-v2/forms/:formKey/:applicationId/manage", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  if (!data.runnerSignInAuthed) {
    // Prototype convenience: if prototype mode is enabled, don't force a sign-in journey.
    if (runnerSignInV2PrototypeMode(req)) {
      applyRunnerSignInV2EmailAuth(req, data.runnerSignInEmail || "you@example.com", data.runnerSignInPhone || "07700 900000");
    } else {
    return res.redirect(
      `/runner-sign-in-v2/sign-in/email?formKey=${encodeURIComponent(req.params.formKey)}&applicationId=${encodeURIComponent(req.params.applicationId)}&next=${encodeURIComponent(runnerSignInV2ManagePath(req.params.formKey, req.params.applicationId))}`
    );
    }
  }
  const applications = ensureRunnerSignInApplications(req);
  ...
  const checkingStatus = String((application.checking && application.checking.status) || "not_started");
  ...
  if (application.status === "Submitted") {
    ...
    v2StatusLabel = "Submitted";
    v2StatusTagClasses = "govuk-tag--green";
  } else if (checkingRequired && checkingStatus === "checked") {
    v2PrimaryAction = "review_submit";
    v2StatusLabel = "Checked";
    v2StatusTagClasses = "govuk-tag--turquoise";
  } else if (checkingRequired && checkingStatus === "awaiting_check") {
    v2StatusLabel = "Awaiting check";
    v2StatusTagClasses = "govuk-tag--yellow";
  }

  const lastUpdatedText =
    application.status === "Submitted" && application.submittedIso
      ? `Submitted ${formatRunnerSignInDate(application.submittedIso)}`
      : formatRunnerSignInLastUpdated(application.updatedIso);
  const expiryText = application.status === "Submitted" ? "—" : formatRunnerSignInDate(application.expiryIso);
  ...
  const checkerInviteUrl = `/runner-sign-in-v2/forms/${encodeURIComponent(application.formKey)}/${encodeURIComponent(application.id)}/checker/invite`;
  // Prototype links for the checker journey.
  if (checkingRequired) {
    if (!application.checking) application.checking = {};
    if (!application.checking.reviewToken) application.checking.reviewToken = createRunnerSignInReviewToken();
    if (!application.checking.referenceNumber) application.checking.referenceNumber = createRunnerV4StyleReferenceNumber();
    if (!application.checking.memorableWord) application.checking.memorableWord = createRunnerSignInMemorableWord();

    // Ensure the checker store is populated so the checker URL works without sending an invite.
    const reviewStore = ensureReviewStore(req);
    const token = String(application.checking.reviewToken || "").trim();
    if (token) {
      const existing = reviewStore.get(token);
      const existingValid = Boolean(existing && runnerSignInV2ReviewEntryValid(req, existing) && existing.checkerV2);
      if (!existingValid) {
        reviewStore.set(token, {
          applicationId: application.id,
          referenceNumber: normalizeRunnerV5ReferenceNumber(application.checking.referenceNumber),
          ...
          reviewDeclarationComplete: false,
          reviewedAtIso: null,
          expires: runnerSignInV2PrototypeMode(req) ? Date.now() + 10 * 365 * 24 * 60 * 60 * 1000 : (Date.now() + 30 * 24 * 60 * 60 * 1000),
          checkerInviteEmail: String((application.checking.inviteEmail || "checker@example.com")).toLowerCase(),
          checkerV2: true,
          checkerV2EmailConfirmed: false,
          checkerV2CodeOk: false,
        });
  ...
  const checkerToken = checkingRequired ? String(application.checking.reviewToken || "") : "";
  const checkerStartUrl = checkerToken ? runnerSignInV2CheckerEmailEntryPath(checkerToken) : "";
  const checkerInviteEmailPreviewUrl = checkingRequired
...
router.get("/runner-sign-in-v2/forms/:formKey/:applicationId/security", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  if (!data.runnerSignInAuthed) {
    if (runnerSignInV2PrototypeMode(req)) {
      applyRunnerSignInV2EmailAuth(req, data.runnerSignInEmail || "you@example.com", data.runnerSignInPhone || "07700 900000");
      ...
      return res.redirect(
        `/runner-sign-in-v2/sign-in/email?formKey=${encodeURIComponent(req.params.formKey)}&applicationId=${encodeURIComponent(req.params.applicationId)}&next=${encodeURIComponent(runnerSignInV2SecurityPath(req.params.formKey, req.params.applicationId))}`
      );
    }
  }
  const application =
  ...
  return res.render("titan-mvp-1.2/runner-sign-in-v2/security", {
    ...
    formKey: application.formKey,
    applicationId: application.id,
    securityUrl: runnerSignInV2SecurityPath(application.formKey, application.id),
    query: req.query,
  });
});

// ── Security: change email (verify via phone) ────────────────────────────────

router.get("/runner-sign-in-v2/forms/:formKey/:applicationId/security/change-email/get-security-code", function (req, res) {
  const ctx = runnerSignInV2EnsureSecuritySession(req, res);
  if (!ctx) return;
  const { data, formKey, applicationId, application, securityUrl } = ctx;
  delete data.runnerSignInV2ChangeEmailPhoneVerified;
  delete data.runnerSignInV2ChangeEmailPending;
  const maskedMobile = runnerSignInV2MaskedMobile(data) || "0000";
  const checkPhoneUrl = runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, "check-phone");
  return res.render("titan-mvp-1.2/runner-sign-in-v2/security/change-email/get-security-code", {
    data,
    application,
    formKey,
    ...
    securityUrl,
    maskedMobile,
    formAction: runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, "get-security-code"),
    backUrl: checkPhoneUrl,
  });
});

router.post("/runner-sign-in-v2/forms/:formKey/:applicationId/security/change-email/get-security-code", function (req, res) {
  const ctx = runnerSignInV2EnsureSecuritySession(req, res);
  if (!ctx) return;
  const { formKey, applicationId } = ctx;
  return res.redirect(`${runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, "check-phone")}?resend=1`);
});

router.get("/runner-sign-in-v2/forms/:formKey/:applicationId/security/change-email/check-phone", function (req, res) {
  const ctx = runnerSignInV2EnsureSecuritySession(req, res);
  if (!ctx) return;
  const { data, formKey, applicationId, application, securityUrl } = ctx;
  const maskedMobile = runnerSignInV2MaskedMobile(data) || "0000";
  const getSecurityCodeUrl = runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, "get-security-code");
  const resend = req.query.resend === "1" || req.query.resend === "true";
  return res.render("titan-mvp-1.2/runner-sign-in-v2/security/change-email/check-phone", {
    data,
    application,
    ...
    maskedMobile,
    getSecurityCodeUrl,
    formAction: runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, "check-phone"),
    resend,
    error: data.runnerSignInV2Error || {},
    values: {},
  });
...
router.post("/runner-sign-in-v2/forms/:formKey/:applicationId/security/change-email/check-phone", function (req, res) {
  const ctx = runnerSignInV2EnsureSecuritySession(req, res);
  if (!ctx) return;
  ...
  if (!/^\d{6}$/.test(code)) {
    data.runnerSignInV2Error = { runnerSignInV2Code: "Enter the 6 digit security code" };
    return res.redirect(runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, "check-phone"));
  }
  delete data.runnerSignInV2Error;
  data.runnerSignInV2ChangeEmailPhoneVerified = true;
...
router.get("/runner-sign-in-v2/forms/:formKey/:applicationId/security/change-email/new-email", function (req, res) {
  if (String(req.query.error || req.query.scenario || "").trim() === "same-email") {
    return res.redirect(RUNNER_SIGN_IN_V2_STATIC_CHANGE_EMAIL_NEW_EMAIL_SAME_AS_CURRENT_PATH);
  }
  const ctx = runnerSignInV2EnsureSecuritySession(req, res);
  if (!ctx) return;
  const { data, formKey, applicationId, application } = ctx;
  if (!data.runnerSignInV2ChangeEmailPhoneVerified) {
    return res.redirect(runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, "get-security-code"));
  }
  return res.render("titan-mvp-1.2/runner-sign-in-v2/security/change-email/new-email", {
    data,
    application,
    ...
    formAction: runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, "new-email"),
    error: data.runnerSignInV2Error || {},
    values: { runnerSignInV2NewEmail: data.runnerSignInV2ChangeEmailPending || "" },
  });
});

router.post("/runner-sign-in-v2/forms/:formKey/:applicationId/security/change-email/new-email", function (req, res) {
  const ctx = runnerSignInV2EnsureSecuritySession(req, res);
  if (!ctx) return;
  const { data, formKey, applicationId } = ctx;
  if (!data.runnerSignInV2ChangeEmailPhoneVerified) {
  ...
  delete data.runnerSignInV2Error;
  data.runnerSignInV2ChangeEmailPending = email;
  return res.redirect(runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, "check-new-email"));
});

router.get("/runner-sign-in-v2/forms/:formKey/:applicationId/security/change-email/check-new-email", function (req, res) {
  const ctx = runnerSignInV2EnsureSecuritySession(req, res);
  if (!ctx) return;
  const { data, formKey, applicationId, application } = ctx;
  if (!data.runnerSignInV2ChangeEmailPending) {
    return res.redirect(runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, "new-email"));
  }
  const resend = req.query.resend === "1" || req.query.resend === "true";
  const newEmailUrl = runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, "new-email");
  return res.render("titan-mvp-1.2/runner-sign-in-v2/security/change-email/check-new-email", {
    data,
    application,
    formKey,
    applicationId,
    email: data.runnerSignInV2ChangeEmailPending,
    newEmailUrl,
    resendUrl: `${runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, "check-new-email")}?resend=1`,
    formAction: runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, "check-new-email"),
    resend,
    error: data.runnerSignInV2Error || {},
    values: {},
  });
...
router.post("/runner-sign-in-v2/forms/:formKey/:applicationId/security/change-email/check-new-email", function (req, res) {
  const ctx = runnerSignInV2EnsureSecuritySession(req, res);
  if (!ctx) return;
  const { data, formKey, applicationId, securityUrl } = ctx;
  if (!data.runnerSignInV2ChangeEmailPending) {
    return res.redirect(runnerSignInV2SecurityChangeEmailPath(formKey, applicationId, "new-email"));
  }
  const code = String(req.body.runnerSignInV2Code || "").replace(/\s+/g, "");
  ...
  delete data.runnerSignInV2Error;
  data.runnerSignInEmail = data.runnerSignInV2ChangeEmailPending;
  delete data.runnerSignInV2ChangeEmailPending;
  delete data.runnerSignInV2ChangeEmailPhoneVerified;
  return res.redirect(`${securityUrl}?emailChanged=1`);
});

// ── Security: change phone (verify via email) ────────────────────────────────

router.get("/runner-sign-in-v2/forms/:formKey/:applicationId/security/change-phone/get-security-code", function (req, res) {
  const ctx = runnerSignInV2EnsureSecuritySession(req, res);
  if (!ctx) return;
  const { data, formKey, applicationId, application, securityUrl } = ctx;
  delete data.runnerSignInV2ChangePhoneEmailVerified;
  const checkEmailUrl = runnerSignInV2SecurityChangePhonePath(formKey, applicationId, "check-email");
  return res.render("titan-mvp-1.2/runner-sign-in-v2/security/change-phone/get-security-code", {
    data,
    application,
    formKey,
    ...
    backUrl: checkEmailUrl,
    formAction: runnerSignInV2SecurityChangePhonePath(formKey, applicationId, "get-security-code"),
  });
});

router.post("/runner-sign-in-v2/forms/:formKey/:applicationId/security/change-phone/get-security-code", function (req, res) {
  const ctx = runnerSignInV2EnsureSecuritySession(req, res);
  if (!ctx) return;
  const { formKey, applicationId } = ctx;
  return res.redirect(`${runnerSignInV2SecurityChangePhonePath(formKey, applicationId, "check-email")}?resend=1`);
});

router.get("/runner-sign-in-v2/forms/:formKey/:applicationId/security/change-phone/check-email", function (req, res) {
  const ctx = runnerSignInV2EnsureSecuritySession(req, res);
  if (!ctx) return;
  const { data, formKey, applicationId, application } = ctx;
  const getSecurityCodeUrl = runnerSignInV2SecurityChangePhonePath(formKey, applicationId, "get-security-code");
  const resend = req.query.resend === "1" || req.query.resend === "true";
  return res.render("titan-mvp-1.2/runner-sign-in-v2/security/change-phone/check-email", {
    data,
    application,
    ...
    email: data.runnerSignInEmail || "you@example.com",
    getSecurityCodeUrl,
    formAction: runnerSignInV2SecurityChangePhonePath(formKey, applicationId, "check-email"),
    resend,
    error: data.runnerSignInV2Error || {},
    values: {},
  });
...
router.post("/runner-sign-in-v2/forms/:formKey/:applicationId/security/change-phone/check-email", function (req, res) {
  const ctx = runnerSignInV2EnsureSecuritySession(req, res);
  if (!ctx) return;
  const { data, formKey, applicationId } = ctx;
  const code = String(req.body.runnerSignInV2Code || "").replace(/\s+/g, "");
  if (!/^\d{6}$/.test(code)) {
    data.runnerSignInV2Error = { runnerSignInV2Code: "Enter the 6 digit security code" };
    return res.redirect(runnerSignInV2SecurityChangePhonePath(formKey, applicationId, "check-email"));
  }
  delete data.runnerSignInV2Error;
  data.runnerSignInV2ChangePhoneEmailVerified = true;
...
router.get("/runner-sign-in-v2/forms/:formKey/:applicationId/security/change-phone/new-phone", function (req, res) {
  const ctx = runnerSignInV2EnsureSecuritySession(req, res);
  if (!ctx) return;
  const { data, formKey, applicationId, application } = ctx;
...
router.post("/runner-sign-in-v2/forms/:formKey/:applicationId/security/change-phone/new-phone", function (req, res) {
  const ctx = runnerSignInV2EnsureSecuritySession(req, res);
  if (!ctx) return;
  const { data, formKey, applicationId, securityUrl } = ctx;
  if (!data.runnerSignInV2ChangePhoneEmailVerified) {
...
router.get("/runner-sign-in-v2/forms/:formKey/:applicationId/ready-to-submit", function (req, res) {
  ...
  ensureRunnerSignInChecking(application);
  const checkingStatus = String((application.checking && application.checking.status) || "not_started");
  if (!application.checking || !application.checking.required || checkingStatus !== "checked") {
    return res.redirect(runnerSignInV2ManagePath(application.formKey, application.id));
  }
  setRunnerSignInV2ManageFocus(req, application.formKey, application.id);
  ...
  const syntheticEntry = entry || {
    formName: application.formName,
    dataSnapshot: application.answers && typeof application.answers === "object" ? application.answers : {},
  };
  const display = runnerSignInV2CheckerCheckAnswersDisplay(syntheticEntry);
...
router.get("/runner-sign-in-v2/forms/:formKey/:applicationId/start-new", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  if (!data.runnerSignInAuthed) {
    if (runnerSignInV2PrototypeMode(req)) {
      applyRunnerSignInV2EmailAuth(req, data.runnerSignInEmail || "you@example.com", data.runnerSignInPhone || "07700 900000");
    } else {
      const backTo = runnerSignInV2ManagePath(req.params.formKey, req.params.applicationId);
      return res.redirect(
        `/runner-sign-in-v2/sign-in/email?formKey=${encodeURIComponent(req.params.formKey)}&applicationId=${encodeURIComponent(req.params.applicationId)}&next=${encodeURIComponent(backTo)}`
      );
    }
  }

  const newId = `app-${Date.now()}`;
  const application = runnerSignInV2EnsureApplication(req, req.params.formKey, newId);
  if (!application) return res.redirect("/runner-sign-in-v2/start-page");
  setRunnerSignInV2ManageFocus(req, application.formKey, application.id);
  return res.redirect(`/runner-sign-in-v2/forms/${encodeURIComponent(application.formKey)}/${encodeURIComponent(application.id)}/start-page`);
});

router.get("/runner-sign-in-v2/save-and-exit/with-sign-in/confirm-email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  ...
  return res.render("titan-mvp-1.2/runner-sign-in-v2/save-and-exit/with-sign-in/confirm-email", {
    ...
    backUrl: manageUrl,
    error: data.runnerSignInV2Error || {},
    values: { runnerSignInV2ResumeEmail: data.runnerSignInEmail || "" },
  });
});

router.post("/runner-sign-in-v2/save-and-exit/with-sign-in/confirm-email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId } = runnerSignInV2ReadTriple(req, req.body);
  const email = String(req.body.runnerSignInV2ResumeEmail || "").trim();
  if (!email) {
    data.runnerSignInV2Error = { runnerSignInV2ResumeEmail: "Enter an email address" };
    return res.redirect(`/runner-sign-in-v2/save-and-exit/with-sign-in/confirm-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}`);
  }
  if (!isRunnerSignInV2ProbablyEmail(email)) {
    data.runnerSignInV2Error = { runnerSignInV2ResumeEmail: "Enter an email address in the correct format, like name@example.com" };
    return res.redirect(`/runner-sign-in-v2/save-and-exit/with-sign-in/confirm-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}`);
  }
  delete data.runnerSignInV2Error;
  return res.redirect(
    `/runner-sign-in-v2/save-and-exit/with-sign-in/leave?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&email=${encodeURIComponent(email)}`
  );
});

router.get("/runner-sign-in-v2/save-and-exit/with-sign-in/leave", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId } = runnerSignInV2ReadTriple(req, req.query);
  const manageUrl = runnerSignInV2ManagePath(formKey, applicationId);
  return res.render("titan-mvp-1.2/runner-sign-in-v2/save-and-exit/with-sign-in/leave", {
...
router.get("/runner-sign-in-v2/save-and-exit/without-sign-in/check-email", function (req, res) {
  ...
  const application = runnerSignInV2ResolveApplication(req, formKey, applicationId);
  const leaveUrl = runnerSignInV2SaveAndExitWithoutSignInLeavePath(formKey, applicationId);
  const resolvedNext = next || leaveUrl;
  if (!data.runnerSignInV2PendingEmail && runnerSignInV2PrototypeMode(req)) {
  ...
  return res.render("titan-mvp-1.2/runner-sign-in-v2/save-and-exit/without-sign-in/check-email", {
    ...
    email: data.runnerSignInV2PendingEmail,
    resend,
    backUrl: `/runner-sign-in-v2/create-sign-in/email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(resolvedNext)}`,
    error: data.runnerSignInV2Error || {},
    values: {},
  });
});
...
router.post("/runner-sign-in-v2/save-and-exit/without-sign-in/check-email", function (req, res) {
  ...
  if (codeError) {
    data.runnerSignInV2Error = { runnerSignInV2Code: codeError };
    return res.redirect(
      `/runner-sign-in-v2/save-and-exit/without-sign-in/check-email?formKey=${encodeURIComponent(formKey)}&applicationId=${encodeURIComponent(applicationId)}&next=${encodeURIComponent(resolvedNext)}`
    );
  }
  delete data.runnerSignInV2Error;
  data.runnerSignInV2CreateSignInEmailConfirmed = true;
...
router.get("/runner-sign-in-v2/save-and-exit/without-sign-in/leave", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const { formKey, applicationId } = runnerSignInV2ReadTriple(req, req.query);
  const manageUrl = runnerSignInV2ManagePath(formKey, applicationId);
  return res.render("titan-mvp-1.2/runner-sign-in-v2/save-and-exit/without-sign-in/leave", {
...
router.get("/runner-sign-in-v2/forms/:formKey/:applicationId/delete", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  if (!data.runnerSignInAuthed) {
    return res.redirect(
      `/runner-sign-in-v2/sign-in/email?formKey=${encodeURIComponent(req.params.formKey)}&applicationId=${encodeURIComponent(req.params.applicationId)}&next=${encodeURIComponent(runnerSignInV2ManagePath(req.params.formKey, req.params.applicationId))}`
...
router.post("/runner-sign-in-v2/forms/:formKey/:applicationId/delete", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  if (!data.runnerSignInAuthed) {
    return res.redirect(
      `/runner-sign-in-v2/sign-in/email?formKey=${encodeURIComponent(req.params.formKey)}&applicationId=${encodeURIComponent(req.params.applicationId)}`
    );
  }
  const choice = String(req.body.deleteApplication || "").trim();
  const targetId = String(req.body.targetApplicationId || req.params.applicationId || "").trim();
  const manageUrl = runnerSignInV2ManagePath(req.params.formKey, req.params.applicationId);
  if (choice !== "yes") return res.redirect(manageUrl);
...
router.get("/runner-sign-in-v2/emails/save-and-exit", function (req, res) {
  const variant = String(req.query.variant || "with-sign-in").trim();
  const formKey = String(req.query.formKey || "").trim();
  const applicationId = String(req.query.applicationId || "").trim();
  const data = ensureRunnerSignInSession(req);
...
router.get("/runner-sign-in-v2/emails/one-login-created", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const toEmail = String(req.query.email || data.runnerSignInEmail || "").trim();
  const formKey = String(req.query.formKey || "").trim();
  const applicationId = String(req.query.applicationId || "").trim();
  const application = runnerSignInV2ResolveApplication(req, formKey, applicationId);
  const formName = (application && application.formName) || "";
  const manageUrl = formKey && applicationId ? `${PUBLIC_BASE_URL}${runnerSignInV2ManagePath(formKey, applicationId)}` : "";
  return res.render("titan-mvp-1.2/runner-sign-in-v2/emails/one-login-created", {
    toEmail,
    formKey,
    applicationId,
...
router.get("/runner-sign-in-v2/emails/email-confirmation-code", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const toEmail = String(req.query.email || data.runnerSignInEmail || "").trim();
  const code = String(req.query.code || "778230").trim();
  const formKey = String(req.query.formKey || "").trim();
  const applicationId = String(req.query.applicationId || "").trim();
  const application = runnerSignInV2ResolveApplication(req, formKey, applicationId);
  const formName = (application && application.formName) || "";
...
router.get("/runner-sign-in-v2/texts/recover-security-code", function (req, res) {
  return res.render("titan-mvp-1.2/runner-sign-in-v2/texts/recover-security-code");
});

router.get("/runner-sign-in-v2/texts/change-email-security-code", function (req, res) {
...
router.post("/runner-sign-in-v2/forms/:formKey/:applicationId/checker/invite", function (req, res) {
  ...
  reviewStore.set(application.checking.reviewToken, {
    ...
    reviewedAtIso: null,
    expires: runnerSignInV2PrototypeMode(req) ? Date.now() + 10 * 365 * 24 * 60 * 60 * 1000 : (Date.now() + 30 * 24 * 60 * 60 * 1000),
    checkerInviteEmail: email.toLowerCase(),
    checkerV2: true,
    checkerV2EmailConfirmed: false,
    checkerV2CodeOk: false,
  });
...
router.get("/runner-sign-in-v2/checker/why-sign-in", function (req, res) {
  ...
  const reviewStore = ensureReviewStore(req);
  const entry = runnerSignInV2EnsureCheckerReviewEntry(req, token, reviewStore);
  const tokenValid = Boolean(entry && runnerSignInV2ReviewEntryValid(req, entry) && entry.checkerV2);
  if (!token || !tokenValid || !entry) {
    return res.redirect(
      token ? `/runner-sign-in-v2/checker/start?token=${encodeURIComponent(token)}&prototype=1` : "/runner-sign-in-v2/checker/start"
    );
  }
  const formKey = entry.formKey;
  const applicationId = entry.applicationId;
  const application = runnerSignInV2ResolveApplication(req, formKey, applicationId);
  const checkerAfterSignInPath = `/runner-sign-in-v2/checker/check-email?token=${encodeURIComponent(token)}`;
  const nextUrl = runnerSignInV2CheckerAppendAllowApplicant(req, checkerAfterSignInPath);
  data.runnerSignInV2CheckerPendingNext = nextUrl;
  const backUrl = runnerSignInV2CheckerAppendAllowApplicant(req, `/runner-sign-in-v2/checker/start?token=${encodeURIComponent(token)}&prototype=1`);
  return res.render("titan-mvp-1.2/runner-sign-in-v2/checker/why-sign-in", {
    data,
    application,
    formKey,
    applicationId,
    formName: entry.formName || (application && application.formName) || "",
    next: nextUrl,
    backUrl,
    token,
  });
...
router.get("/runner-sign-in-v2/checker/start", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const token = String(req.query.token || "").trim();
  if (!token) {
    return res.render("titan-mvp-1.2/runner-sign-in-v2/checker/landing", {
  ...
  const reviewStore = ensureReviewStore(req);
  let entry = runnerSignInV2EnsureCheckerReviewEntry(req, token, reviewStore);
  const tokenValid = Boolean(entry && runnerSignInV2ReviewEntryValid(req, entry));
  ...
  if (!tokenValid || !entry || !entry.checkerV2) {
    return res.render("titan-mvp-1.2/runner-sign-in-v2/checker/landing", {
      data: {},
      formName: entry && entry.formName ? entry.formName : "This form",
      continueUrl: "#",
      tokenValid: false,
    });
  }
  return res.render("titan-mvp-1.2/runner-sign-in-v2/checker/landing", {
    ...
    continueUrl: runnerSignInV2CheckerAppendAllowApplicant(
      req,
      `/runner-sign-in-v2/checker/why-sign-in?token=${encodeURIComponent(token)}&prototype=1`
    ),
    tokenValid: true,
  });
});

router.get("/runner-sign-in-v2/checker/confirm-email", function (req, res) {
  const token = String(req.query.token || "").trim();
  if (!token) {
    return res.redirect("/runner-sign-in-v2/checker/start");
  }
  return res.redirect(
    runnerSignInV2CheckerAppendAllowApplicant(req, `/runner-sign-in-v2/checker/check-email?token=${encodeURIComponent(token)}`)
  );
});

router.post("/runner-sign-in-v2/checker/confirm-email", function (req, res) {
...
router.get("/runner-sign-in-v2/checker/check-email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const token = String(req.query.token || "").trim();
  if (!token) {
    return res.redirect("/runner-sign-in-v2/checker/start");
  ...
  // Invited email is stored on the token; no separate “confirm invite email” screen.
  if (!entry.checkerV2EmailConfirmed) {
    entry.checkerV2EmailConfirmed = true;
    reviewStore.set(token, entry);
  }
  return res.render("titan-mvp-1.2/runner-sign-in-v2/checker/check-email", {
    ...
    token,
    allowApplicant: runnerSignInV2CheckerAllowApplicant(req),
    checkerEmail: entry.checkerInviteEmail,
    backUrl: runnerSignInV2CheckerAppendAllowApplicant(
      req,
      `/runner-sign-in-v2/checker/why-sign-in?token=${encodeURIComponent(token)}&prototype=1`
    ),
    error: data.runnerSignInV2CheckerError || {},
    values: {},
  });
});

router.post("/runner-sign-in-v2/checker/check-email", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  if (data.runnerSignInAuthed && !runnerSignInV2CheckerAllowApplicant(req)) {
    return res.render("titan-mvp-1.2/runner-sign-in/check/applicant-blocked", { data, signOutLink: "/titan-mvp-1.2/roles/sign-out" });
  }
  ...
  if (!/^\d{6}$/.test(code)) {
    data.runnerSignInV2CheckerError = { runnerSignInV2Code: "Enter the 6 digit security code" };
    return res.redirect(runnerSignInV2CheckerAppendAllowApplicant(req, `/runner-sign-in-v2/checker/check-email?token=${encodeURIComponent(token)}`));
  }
  delete data.runnerSignInV2CheckerError;
  entry.checkerV2CodeOk = true;
  reviewStore.set(token, entry);
  return res.redirect(runnerSignInV2CheckerAppendAllowApplicant(req, `/runner-sign-in-v2/checker/check-answers?token=${encodeURIComponent(token)}`));
});

router.get("/runner-sign-in-v2/checker/review-declaration", function (req, res) {
  const token = String(req.query.token || "").trim();
  if (!token) return res.redirect("/runner-sign-in-v2/checker/start");
  return res.redirect(
    runnerSignInV2CheckerAppendAllowApplicant(req, `/runner-sign-in-v2/checker/check-answers?token=${encodeURIComponent(token)}`)
  );
});

router.get("/runner-sign-in-v2/checker/check-answers", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  const token = String(req.query.token || "").trim();
  if (!token) {
    return res.redirect("/runner-sign-in-v2/checker/start");
...
router.get("/runner-sign-in-v2/checker/change-answers", function (req, res) {
  ...
  return res.render("titan-mvp-1.2/runner-sign-in-v2/checker/change-answers", {
    ...
    token,
    allowApplicant: runnerSignInV2CheckerAllowApplicant(req),
    backUrl: runnerSignInV2CheckerAppendAllowApplicant(req, `/runner-sign-in-v2/checker/check-answers?token=${encodeURIComponent(token)}`),
    checkAnswersUrl: runnerSignInV2CheckerAppendAllowApplicant(req, `/runner-sign-in-v2/checker/check-answers?token=${encodeURIComponent(token)}`),
  });
});

router.post("/runner-sign-in-v2/checker/complete", function (req, res) {
  const data = ensureRunnerSignInSession(req);
  if (data.runnerSignInAuthed && !runnerSignInV2CheckerAllowApplicant(req)) {
    return res.render("titan-mvp-1.2/runner-sign-in/check/applicant-blocked", { data, signOutLink: "/titan-mvp-1.2/roles/sign-out" });
  }
  ...
  if (!runnerSignInV2CheckerDeclarationConfirmed(req.body)) {
    return renderRunnerSignInV2CheckerCheckAnswers(req, res, {
      token,
      entry,
      error: { declarationError: "You must accept the declaration to continue" },
    });
  }
  entry.reviewDeclarationComplete = true;
  entry.reviewedAtIso = new Date().toISOString();
  ...
  if (application && application.checking) {
    application.checking.status = "checked";
    application.checking.checkedAtIso = entry.reviewedAtIso;
    application.checking.checkedBy = entry.checkedBy;
  }

  return res.redirect(runnerSignInV2CheckerAppendAllowApplicant(req, `/runner-sign-in-v2/checker/complete/success?token=${encodeURIComponent(token)}`));
});

router.get("/runner-sign-in-v2/checker/complete/success", function (req, res) {
  const token = String(req.query.token || "").trim();
  const reviewStore = ensureReviewStore(req);
  const entry = token ? reviewStore.get(token) : null;
  const formName = (entry && entry.formName) || "Apply for a small grant";
...
router.get("/runner-sign-in-v2/emails/checker-invite-v2", function (req, res) {
  const formKey = String(req.query.formKey || "").trim();
  const applicationId = String(req.query.applicationId || "").trim();
  const applications = ensureRunnerSignInApplications(req);
  const application = applications.find((a) => a.id === applicationId && a.formKey === formKey);
  const token = application && application.checking ? application.checking.reviewToken : "";
  const toEmail = String(
    (application && application.checking && application.checking.inviteEmail) || req.query.email || ""
  ).trim();
  const formName = (application && application.formName) || "";
  const checkerUrl = token ? `${PUBLIC_BASE_URL}${runnerSignInV2CheckerEmailEntryPath(token)}` : "";
  return res.render("titan-mvp-1.2/runner-sign-in-v2/emails/checker-invite-v2", {
    toEmail,
    formName,
    checkerUrl,
...
router.get("/runner-sign-in-v2/emails/applicant-form-checked", function (req, res) {
  const token = String(req.query.token || "").trim();
  const reviewStore = ensureReviewStore(req);
  const entry = token ? reviewStore.get(token) : null;
  const applications = ensureRunnerSignInApplications(req);
  const application = entry ? applications.find((a) => a.id === entry.applicationId) : null;
  const readyToSubmitUrl = application
    ? `${PUBLIC_BASE_URL}/runner-sign-in-v2/forms/${encodeURIComponent(application.formKey)}/${encodeURIComponent(application.id)}/ready-to-submit`
    : "";
  const data = ensureRunnerSignInSession(req);
  const formName = (application && application.formName) || (entry && entry.formName) || "";
  return res.render("titan-mvp-1.2/runner-sign-in-v2/emails/applicant-form-checked", {
    toEmail: String(data.runnerSignInEmail || "").trim(),
    formName,
    readyToSubmitUrl,
  });
});
